...
__all__ = ["fbanks", "LPC", "create_lngclf_nn", "LngDataset"]

# implementation of the low-pass filter


def lowPassFilter(signal, coeff=0.97):
    # y[n] = x[n] - a*x[n-1] , a = 0.97 , a>0 for low-pass filters
...
def LPC(signal, rate):
    ...
    EmphasizedTime = np.linspace(
        0, len(emphasizedSignal)/rate, num=len(emphasizedSignal))
...
class LngDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        X = LPC(signal, sample_rate).astype(np.float32)
        X = X[np.newaxis, ...]
        return X, y
...
def main(archive, epochs=10):
    ...
    for epoch, train_loss, validation_loss in islice(loop(net, trainer, criterion, train_loader, test_loader), epochs):
        print(
            f"EPOCH {epoch} END - TRAIN_LOSS={train_loss}, VALIDATION_LOSS={validation_loss}")
...
def cli(parser):
    ...
    infer_parser = subparsers.add_parser(
        "infer", help="Infer using trained model")

    train_parser.add_argument(
        "-e", "--epochs", required=True, default=10, type=int, help="Number of epochs")
    train_parser.add_argument("archive", help="Dataset archive")
    train_parser.set_defaults(
        function=lambda args: main(args.archive, args.epochs))

    infer_parser.add_argument(
        "-m", "--model", required=True, help="Model to use")
    infer_parser.add_argument(
        "audio", nargs="+", help="Audio file(s) to process")
    infer_parser.set_defaults(
        function=lambda args: infer(args.model, args.audio))
